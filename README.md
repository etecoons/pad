...
![rustpad](https://github.com/user-attachments/assets/3d880a0b-bbee-4794-ae2a-1b7b79335dde)
...
services:
  rustpad:
    ...
    environment:
      ...
      BASE_URL: ${RUSTPAD_BASE_URL:-http://localhost:3000} # Use ALLOWED_ORIGINS below to restrict cors to specific origins
      # (OPTIONAL)
      ...
      # ALLOWED_ORIGINS: ${RUSTPAD_ALLOWED_ORIGINS:-http://localhost:3000} # Comment out to allow all origins (*)
      ...
      # By default, RustPad includes support for all ~180 languages supported by highlight.js.
...
RUSTPAD_PIN=1234          # Optional PIN protection
SITE_TITLE=RustPad        # Custom site title
...
# If using a custom path like /opt/docker/rustpad
sudo chown -R 1000:1000 /opt/docker/rustpad

# For Unraid users
sudo chown -R 1000:1000 /mnt/user/appdata/rustpad
```
...
3. **Verify file ownership inside container:**
   ```bash
   docker exec rustpad ls -la /app/data
   ```
...
| RUSTPAD_PIN             | PIN protection (4-10 digits)                                 | None                  | No       |
| SITE_TITLE              | Site title displayed in header                               | RustPad               | No       |
...
When deploying RustPad behind a reverse proxy (nginx, Apache, Cloudflare, etc.), you may need to configure proxy trust to correctly identify client IP addresses for rate-limiting and authentication.
...
```bash
# While RustPad is running, check who's connecting
...
| **Default Markdown Preview**   | Default view when loading RustPad (Client-based)         | Editor   | Editor, Split, Full       |
...
RustPad now supports enhanced markdown features:
