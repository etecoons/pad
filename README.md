...
# <img src="assets/icon.png" width="32" height="32" valign="middle"> Pad

Secure, self-hosted, collaborative markdown text notepad editor.
...
docker run -d \
  ...
  -e PAD_PIN=your_secret_pin \
  ghcr.io/studio2201/pad:latest
```
...
| :--- | :--- | :--- |
| `PORT` | The network port the web server binds to | `4402` |
| `PAD_PIN` | Security PIN code required for client authentication | (None) |
| `PAD_DATA_DIR` | Directory path where persistent data is stored | `/config` |
...
Each container includes a built-in admin tool located in the system path as `pad`. To open the console, execute a shell inside the container:
```bash
docker exec -it <container-name> sh
```
Then, run `pad` to manage the application:
```bash
pad [command]
```
Running `pad` without arguments or running `pad tui` launches the interactive terminal user interface.
