# =============================================================================
# pad — Alpine container image (Unraid-compatible)
# =============================================================================
...
#     PAD_DATA_DIR env var if needed.
#   - Listens on 0.0.0.0:<port> for bridge networking.
#
# Build (Podman):
#   cd ~/projects/studio2201/pad
#   podman build -f Containerfile.ubi -t pad:latest .
#
# Run (Unraid-style):
#   podman run -d --name pad \
#     -p <host-port>:4402 \
#     -v /mnt/user/appdata/pad:/config \
#     -e PAD_PIN=<your-pin> \
#     pad:latest
#

# Compare size:
#   podman images | grep -E 'pad|studio2201/pad'
# =============================================================================
...
LABEL org.opencontainers.image.title="pad" \
      org.opencontainers.image.description="pad (Alpine, built on Alpine; Unraid-compatible)" \
      org.opencontainers.image.source="https://github.com/studio2201/pad" \
...
COPY --from=builder /src/target/release/sh /usr/local/bin/pad
RUN chmod +x /usr/local/bin/pad
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD wget -qO- "http://127.0.0.1:${PORT:-4402}/health" >/dev/null || exit 1
